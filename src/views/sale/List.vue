...
<script>
import { getSaleList, getSaleDetail } from '@/api/order'
...
export default {
  ...
  computed: {
    ...mapGetters(['salePage', 'saleQuery'])
  },
  created() {
    this.page = JSON.parse(JSON.stringify(this.salePage))
    this.queryData = JSON.parse(JSON.stringify(this.saleQuery))
    const { current, size } = this.page
    this.fetchData(current, size)
  },
  beforeDestroy() {
    this.SET_SALE_PAGE(this.page)
    this.SET_SALE_QUERY(this.queryData)
  },
  methods: {
    ...mapMutations('lists', ['SET_SALE_PAGE', 'SET_SALE_QUERY']),
    fetchData(page, numberPerPage) {
      this.listLoading = true
      getSaleList(page, numberPerPage, this.queryData).then(data => {
    ...
    getProductSaleData(id) {
      ...
      getSaleDetail({ productID: id, startDate, endDate }).then(data => {
